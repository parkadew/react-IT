...
export const store = configureStore({
    ...
    /*
        ...
        env 는 "환경변수" 를 의미한다  -->  환경변수 : 코드의 동작에 영향을 미치는 변수, 동적으로 변경하는 값이다 (시스템 환경에 따라 값이 변경될 수 있다는 의미이다)
                                                          -->  일반변수는 선언할 때 "코드" 안에다가 선언하지만, 환경변수는 "컴퓨터" 에다가 선언하는 변수이다 (production / development 이다)
        ...
        * 하지만 우리가 개발을 하고 있을 때는 "npm start" 로 실행을 한다
        ...
        ==>  이런 식으로 env 의 값은 환경에 따라 변한다 (개발자모드이냐, 실제로 사용자들에게 배포된 모드이냐에 따라 달라진다)

        devtools 는 개발자 도구이다  -->  즉, 실제 사용자들이 보는 production 모드일 때는 이 devtools 가 활성화 되면 안된다 (개발 모드일 때만 개발자모드가 활성화 되어야 한다)
        ==>  즉, NODE_ENV !== "production" 를 해석해보면  -->  사용자모드가 아닐 때 devtools 를 켠다는 의미이다
    */
