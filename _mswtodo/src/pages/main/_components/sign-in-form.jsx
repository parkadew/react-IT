...
const SignInForm = () => {
    ...
    const handlePressSignIn = async (data) => { //-->  async 로 비동기 함수라고 정의했다 (이제 이 함수는 Promise 객체를 반환한다)
        ...
        try { //==> 에러 핸들링 하기 위해 try , catch 사용해준 것이다 (백엔드에서 status 가 실패를 반환하면 에러처리 부분인 catch 로 이동한다)
            ...
            // "/api/user/login"  -->  백엔드가 정해준 post 메소드로 로그인에 관한 데이터를 보내면 되는 것이다  -->  그 후엔 성공이나 실패에 관한 데이터로 res 받게된다
            ...
            const response = await fetch("/api/user/login", {
                ...
                /*
                    ...
                    fetch :
                        ...
                        -->  ex) const name = await response.json()  -->  이렇게 바꿔줘야한다 (json 함수도 비동기이기 때문에 반드시 await 걸어줘야한다)
                        -->  fetch 로 백엔드에 데이터 보낼 때는 stringify 형태로 전달해야한다

                    await :
                        -->  await 은 비동기에서 "기다려" 라고 명령을 내린 부분이다  -->  "비동기 처리"
                        ...
                        -->  await 은 자바스크립트에서 비동기 함수 (async function) 내에서 사용되는 키워드로, 프로미스(Promise)가 해결될 때까지 함수의 실행을 중지하고, 프로미스가
                                해결되면 결과 값을 반환하다  -->  이는 비동기 코드를 "동기식" 코드처럼 읽고 작성할 수 있게 도와준다  -->  "비동기 처리" 해준 것이다
                        -->  await 은 오직 async function (비동기 함수) 내부에서만 사용할 수 있다  -->  일반 함수에서는 사용할 수 없다
                        -->  await 은 응답이 올 때까지 기다리는 것이다 (동기적인 코드를 비동기 처리 해주는 것이다)

                    method :
                        같은 이름 ("/api/user/login") 이어도 "재사용" 할 수 있게 해주는 친구이다  -->  post(등록) , get(조회) , push(수정) , patch(수정) , delete(삭제)
                        -->  이렇게 메소드에 따라 같은 주소 ("/api/user/login") 여도 다른 기능을 할 수 있도록 만들어 놓은 것이다  -->  투두 페이지에서 기능이 많기에 제대로 배운다
                        -->  즉, 같은 이름 (같은 백엔드 주소) 이어도 메소드를 다르게 설정하면, 같은 이름으로 여러개 만들 수 있는 것이다 (get, post, patch ... 등등)
                        -->  이 메소드를 정하는 주체는 "백엔드" 개발자이다
                        -->  즉, 백엔드는 먼저 데이터를 전달받을 자기 "주소" 를 정하고, 그 주소에 어떤 "메소드" 로 데이터를 전달받을지 정하면 되는 것이다
                                (메소드마다 프론트에게 전달하는 응답값이 달라진다  -->  즉, 전달받는 response 가 달라지는 것이다)
            ...
            }) // .then((res) => res.json())  -->  위에 확인해보면 const response 에 이어지는 코드이다 (위에 이미 await 적어 놨기에 안 적어도 된다)
            //-->  이렇게 적으면 받아온 데이터를 json 으로 바꾸는 것 까지 상단에 적어놓은 await 이 적용되기에, 아래 response_data 처럼 await 한번 더 안 적어도 된다
            //-->  굳이 이렇게 안하고, 아래 response_data 처럼 await 걸어 놓고 사용해도 된다 (async 함수이기에, 새로 변수 만들고 await 걸어주는 것이 좋다)
            ...
            //==>  fetch 로 응답받은 데이터는 "문자열" (스트링) 형태로 오기 때문에 json() 시켜서 "객체" 로 바꿔준 것인가?
            //==>  user.api.js 와 함께 공부해보자

            // fetch 에서 받아온 데이터는 string 형태로 오기 때문에, json 으로 바꿔줘야한다
            // then 안써준다면 아래처럼 바꿔주면 된다  -->  이렇게 해줘도 된다 (이렇게 await 두번 걸어도 상관 없다  -->  async 함수이기에 await 걸어주는 것이 정석이다)
            const response_data = await response.json()
            if (response_data.status === 200) {
                navigate("/todo/3")  //-->  성공했다면 투두페이지로 이동하는 것이다
                //-->  성공했다면, 백엔드에서는 "token" 을 준다 (원래는 유저마다 고유번호를 준다  -->  사이트의 "출입증" 이 되는 것이다)
            }
